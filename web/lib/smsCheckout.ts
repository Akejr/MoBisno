/**
 * Compra de pacotes de SMS de confirmação. O valor é da plataforma (mesma chave
 * MoMenu dos planos). O saldo é creditado no servidor após o pagamento. Para
 * testar sem cobrar, abra o painel com `?qa=1`.
 */
import { esc, formatKz, toast } from "./dom.js";
import { initPayment, checkStatus } from "./paymentsApi.js";
import { SMS_UNIT_PRICE } from "../supabase/sms.js";

const ACCENT = "#F95901";

export function openSmsCheckout(opts: { ownerId: string; storeId: string; quantity: number; onPaid: () => void }): void {
  const { ownerId, storeId, quantity, onPaid } = opts;
  const amount = quantity * SMS_UNIT_PRICE;
  const qa = location.search.includes("qa=1");
  let method: "mcx" | "reference" = "mcx";

  document.getElementById("mb-sms-checkout")?.remove();
  const host = document.createElement("div");
  host.id = "mb-sms-checkout";
  ...
  function drawForm(): void {
    card.innerHTML = `
      <div class="flex items-center justify-between mb-1">
        <h3 class="text-lg font-black text-gray-900">Comprar ${quantity} SMS</h3>
        <button data-close class="text-gray-400 hover:text-gray-700"><span class="material-symbols-outlined">close</span></button>
      </div>
      <p class="text-gray-500 text-sm mb-4">${quantity} mensagens × ${esc(formatKz(SMS_UNIT_PRICE))} = <b style="color:${ACCENT}">${esc(formatKz(amount))}</b>.</p>
      <div class="space-y-2 mb-4">
        ${methodCard("mcx", "smartphone", "Multicaixa Express", "Crédito imediato.")}
      ...
      <label class="block mb-4" data-phone-wrap>
        ...
        <input id="sms-phone" type="tel" placeholder="9XX XXX XXX" class="mt-1 w-full bg-white border border-gray-300 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[#F95901]" />
      </label>
      ${qa ? `<p class="text-xs rounded-lg px-3 py-2 mb-3" style="background:rgba(0,0,0,.05)">Modo de teste (QA) ativo — sem cobrança real.</p>` : ""}
      <button id="sms-pay" class="w-full py-3 rounded-xl text-white font-bold inline-flex items-center justify-center gap-2" style="background:${ACCENT}"><span class="material-symbols-outlined text-[20px]">bolt</span> Pagar e creditar</button>`;
    bindForm();
  }
  ...
  function bindForm(): void {
    ...
    card.querySelector("#sms-pay")?.addEventListener("click", () => void pay());
  }

  async function pay(): Promise<void> {
    const phone = (card.querySelector("#sms-phone") as HTMLInputElement | null)?.value.trim() || "";
    if (method === "mcx" && phone.replace(/\D/g, "").length < 9) { toast("Indique um telemóvel válido.", "error"); return; }
    const btn = card.querySelector("#sms-pay") as HTMLButtonElement | null;
    if (btn) { btn.disabled = true; btn.style.opacity = "0.7"; btn.textContent = "A processar…"; }

    const res = await initPayment({
      kind: "sms",
      ownerId, storeId, smsQuantity: quantity, method,
      products: [{ productName: `${quantity} SMS de confirmação (MôBisno)`, productPrice: SMS_UNIT_PRICE, productQuantity: quantity }],
    ...
    if (!res.success) {
      if (btn) { btn.disabled = false; btn.style.opacity = "1"; btn.innerHTML = `<span class="material-symbols-outlined text-[20px]">bolt</span> Pagar e creditar`; }
  ...
  function drawDone(): void {
    card.innerHTML = `<div class="text-center py-6">
      ...
      <h3 class="text-xl font-black mt-3">${quantity} SMS creditados!</h3>
      <p class="text-gray-500 text-sm mt-1">O saldo da sua loja foi atualizado.</p>
      <button id="sms-finish" class="mt-5 w-full py-3 rounded-xl text-white font-bold" style="background:${ACCENT}">Concluir</button>
    </div>`;
    card.querySelector("#sms-finish")?.addEventListener("click", () => { close(); onPaid(); });
  }
  ...
  function drawReference(res: Awaited<ReturnType<typeof initPayment>>): void {
    ...
    card.innerHTML = `
      ...
      <p id="sms-ref-status" class="text-center text-sm text-gray-500 mt-3">Os SMS são creditados assim que o pagamento for confirmado.</p>
      <button id="sms-check" class="mt-2 w-full py-3 rounded-xl text-white font-bold inline-flex items-center justify-center gap-2" style="background:${ACCENT}"><span class="material-symbols-outlined text-[20px]">refresh</span> Já paguei — verificar</button>`;
    card.querySelector("[data-close]")?.addEventListener("click", close);
    const operationId = res.operationId ?? "";
    const mtx = res.transactionId ?? "";
    let busy = false;
    card.querySelector("#sms-check")?.addEventListener("click", async () => {
      if (busy || !operationId) return;
      busy = true;
      const st = await checkStatus({ operationId, merchantTransactionId: mtx, storeId });
      busy = false;
      const el = card.querySelector("#sms-ref-status");
